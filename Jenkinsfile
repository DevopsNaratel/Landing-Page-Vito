        ...
        stage('Checkout & Versioning') { 
            steps { 
                ...
                script { 
                    ...
                    def commitHash = sh(returnStdout: true, script: "git rev-parse --short 
HEAD").trim() 
            ...
            steps { 
                script { 
                    docker.withRegistry('', DOCKER_CREDS) { 
                        ...
                        def appImage = 
docker.build("${DOCKER_IMAGE}:${env.BASE_TAG}") 
                        appImage.push() 
...
usernameVariable: 'GIT_USER', passwordVariable: 'GIT_PASS')]) { 
                            ...
                            sh "git clone 
https://${GIT_USER}:${GIT_PASS}@${MANIFEST_REPO_URL} ." 
                            sh 'git config user.email "jenkins@bot.com"' 
                            sh 'git config user.name "Jenkins Pipeline"' 
                             
                            // Mengupdate tag image di file deployment.yaml 
                            sh "sed -i -E 's|image: (docker.io/)?${DOCKER_IMAGE}:.*|image: 
docker.io/${DOCKER_IMAGE}:${env.BASE_TAG}|g' ${MANIFEST_TEST_PATH}" 
                             
                            sh """ 
                                git add . 
                                if ! git diff-index --quiet HEAD; then 
                                    git commit -m 'Deploy Calculator: ${env.BASE_TAG} [skip 
ci]' 
        ...
        stage('Approval for Prod') { 
            steps { 
                input message: "Aplikasi sudah di-update di DEV. Lanjut update manifest 
PROD?", ok: "Promote to Prod" 
            } 
        } 
...
usernameVariable: 'GIT_USER', passwordVariable: 'GIT_PASS')]) { 
                            ...
                            sh "sed -i -E 's|image: (docker.io/)?${DOCKER_IMAGE}:.*|image: 
docker.io/${DOCKER_IMAGE}:${env.BASE_TAG}|g' ${MANIFEST_PROD_PATH}" 
                             
                            sh """ 
                                git add . 
                                if ! git diff-index --quiet HEAD; then 
                                    git commit -m 'Promote Calculator to PROD: 
